    ...
    for path in list_of_ims:
        ...
        if n_det > 0:
            for idx, (corners3D, center_cam, center_2D, dimensions, pose, score, cat_idx) in enumerate(zip(
                ...
                dimensions_str = ', '.join(['{:.2f}'.format(dim) for dim in dimensions.tolist()])
                center_cam_str = ', '.join(['{:.2f}'.format(dim) for dim in center_cam.tolist()])
                meshes_text.append('{} {:.2f} | Dimensions: {} | Centercam: {}'.format(cat, score, dimensions_str, center_cam_str))
